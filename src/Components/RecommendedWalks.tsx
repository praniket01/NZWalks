...
const RecommendedWalks = () => {
    ...
    const setSavedWalks = useAuthStore((state) => state.setSavedWalks);

    useEffect(() => {
        const fetchedSavedWalks = async () => {
           const res =   await axiosInstance.get('walks/saved', {
                headers: {
                    Authorization: 'Bearer ' + token
                }
            })

            setSavedWalks(res.data);
        }
